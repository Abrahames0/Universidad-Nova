...
export default function PersonaUpdateForm(props) {
  const {
    id: idProp,
    persona: personaModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = personaRecord
      ? { ...initialValues, ...personaRecord }
      : initialValues;
    setNombre(cleanValues.nombre);
    setApellidoPaterno(cleanValues.apellidoPaterno);
    setApellidoMaterno(cleanValues.apellidoMaterno);
    setParentesco(cleanValues.parentesco);
    setErrors({});
  };
  const [personaRecord, setPersonaRecord] = React.useState(personaModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Persona, idProp)
        : personaModelProp;
      setPersonaRecord(record);
    };
    queryData();
  }, [idProp, personaModelProp]);
  React.useEffect(resetStateValues, [personaRecord]);
  const validations = {
    nombre: [],
    apellidoPaterno: [],
    apellidoMaterno: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Persona.copyOf(personaRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "PersonaUpdateForm")}
      {...rest}
    >
      <TextField
        label="Nombre"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || personaModelProp)}
